...
describe("StreakDashboard Archive UI Affordance", () => {
  afterEach(() => {
    if (root) {
      act(() => {
        root?.unmount();
  ...
  it("renders the Archive button and shows confirmation on click", async () => {
    setupDom();
    ...
    assert.ok(archiveTrigger, "Archive trigger should be visible");
    
    await clickElement(archiveTrigger);
    
    assert.equal(
      archiveTrigger.getAttribute("data-state"), 
      "open", 
